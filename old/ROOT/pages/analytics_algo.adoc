...
image::algo_life_cycle.svg["Algo Life Cycle"]
...
[latexmath]
++++
SlippageToArrivalMidTrade = 10^6×I_{= 1   if  Buy}^{= -1 if Sell}\\
×\frac{(AllInExecutionPrice-ArrivalPrice)}{ArrivalPrice}
++++
...
[latexmath]
++++
10^{6}×\frac{(1.205-1.20)}{1.20} = 4166 $/m.
++++
...
[latexmath]
++++
SlippageToArrivalMidRemain = 10^6×I_{= 1  if  Buy}^{= -1 if Sell}\\

×\frac{(LastMidPrice-ArrivalPrice)}{ArrivalPrice}
++++
...
[latexmath]
++++
10^{6}×\frac{(1.21-1.20)}{1.20} = 8333 $/m.
++++
...
[latexmath]
++++
SlippageToArrivalMid = SlippageToArrivalMidRemain × \frac{RemainQuantity}{OrderQuantity}\\
	+ \\
    \\  SlippageToArrivalMidTrade × \frac{TradeQuantity}{OrderQuantity}
++++
...
[latexmath]
++++
10^{6}×(\frac{(1.205-1.20)}{1.20} ×0.9 + \frac{(1.21-1.20)}{1.20} ×0.1) = 4583 $/m.
++++
...
[latexmath]
++++
ArrivalMidPerfTrade(BPS) =
10^4×I_{= 1  if  Buy}^{= -1 if Sell}
\\×\frac{(ArrivalPrice-AllInExecutionPrice)}{ArrivalPrice}
++++
...
[latexmath]
++++
10^{4}× \frac{(1.20-1.205)}{1.20} = -41.66 bps.
++++
...
[latexmath]
++++
ArrivalMidPerfTrade(BPS) = -0.01 × SlippageToArrivalMidTrade(PM)
++++
...
[latexmath]
++++
ArrivalMidPerfRemain(BPS)=10^4 × I_{=1  if  Buy}^{=-1 if Sell}
\\× \frac{ArrivalMidPrice - LastMidPrice}{ArrivalMidPrice}
++++
...
[latexmath]
++++
10^{4}× \frac{(1.20-1.21)}{1.20} = -83.33 bps.
++++
...
[latexmath]
++++
ArrivalMidPerfRemain(BPS) = −0.01 × SlippageToArrivalMidRemain(PM)
++++
...
++++
ArrivalMidPerf=
ArrivalMidPerfRemain×\frac{RemainQuantity}{OrderQuantity}\\
+\\
ArrivalMidPerfTrade×\frac{TradeQuantity}{OrderQuantity}
++++
...
[latexmath]
++++
10^{4}×(\frac{(1.20-1.205)}{1.20} ×0.9 + \frac{(1.20-1.21)}{1.20} ×0.1) = -45.83 bps.
++++
...
[latexmath]
++++
ArrivalMidPerf= −0.01 × SlippageToArrivalMid
++++
...
image::algo_ex_reversal_score.png["Algo Life Cycle"]
...
[latexmath]
++++
SlippageToTWAPMid(PM)= 10^6 × I_{=1  if  Buy}^{=-1 if Sell}
\\× \frac{AllInExecutionPrice - TWAPMidPrice}{TWAPMidPrice}
++++
...
[latexmath]
++++
SlippageToRiskTransferPrice(PM)= 10^6 × I_{=1  if  Buy}^{=-1 if Sell}
\\ × \frac{AllInExecutionPrice - RiskTransferPrice}{RiskTransferPrice}
++++
...
[latexmath]
++++
RiskTransferPricePerfBPS= 10^4 × I_{=1  if  Buy}^{=-1 if Sell}
\\ ×  \frac{RiskTransferPrice - AllInExecutionPrice}{RiskTransferPrice}
++++
...
++++
Saving USD=
 \frac{RiskTransferPricePerfBPS × TradeQuantityUSD}{10^4}
++++
...
[latexmath]
++++
AssumedRisk= \sqrt( \sum_{t=1}^{T} \sigma_{t}^{2})
++++
...
[latexmath]
++++
SpreadPaid (PM)= 10^6 × I_{=1  if  Buy}^{=-1 if Sell} ×  \frac{(Price-mid)}{mid}
++++
...
[latexmath]
++++
SpreadPaid (PM)=10^4 × I_{=1  if  Buy}^{=-1 if Sell} ×  \frac{(Price-mid)}{mid}
++++
...
[latexmath]
++++
10^6 × \frac{(1.2001-1.20)}{1.20} = 83.3 $/𝑚.
++++
...
[latexmath]
++++
MarkoutPM(t)= 10^6× I_{=1  if  Buy}^{=-1 if Sell} ×    \frac{mid_t-mid_0}{mid _0}
++++
...
[latexmath]
++++
MarkoutPnLPM(t)= 10^6 × I_{=1  if  Buy}^{=-1 if Sell} × \frac{Price - mid_t}{mid _0}
++++
